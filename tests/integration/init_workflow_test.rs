...
fn test_complete_init_workflow_default() {
    ...
    let mut cmd = Command::cargo_bin("sprit-mutil");
    cmd.current_dir(&temp_path)
        .arg("init")
        .assert()
        .success()
        .stdout(predicates::contains("🚀 Initializing Sprite multi-agent environment"))
        .stdout(predicates::contains("✅ Sprite environment initialized successfully"))
        .stdout(predicates::contains("Created directories:"));
...
fn test_init_workflow_custom_agent_count() {
    ...
    cmd.current_dir(&temp_path)
        ...
        .assert()
        .success()
        .stdout(predicates::contains("🤖 Configured 5 agents:"));
...
fn test_init_workflow_with_force() {
    ...
    let mut cmd_fail = Command::cargo_bin("sprit-mutil");
    ...
    let mut cmd_force = Command::cargo_bin("sprit-mutil");
...
fn test_init_workflow_not_git_repository() {
    ...
    cmd.current_dir(&temp_path)
        ...
        .assert()
        .failure()
        .stderr(predicates::contains("Failed to validate git repository"));
}
...
fn test_init_workflow_all_file_types() {
    ...
    for entry in walkdir::WalkDir::new(&agents_dir).into_iter().flatten() {
        if entry.file_type().is_file() {
            created_files.push(entry.path().strip_prefix(&temp_path).unwrap().to_string_lossy().to_string());
        }
    }
    ...
    let expected_files = vec![
        ...
        "agents/scripts/attach-session.sh",
        "agents/profiles/profile0.sh",
    ...
    for expected_file in &expected_files {
        assert!(created_files.contains(&expected_file.to_string()),
               "Expected file {} should be created", expected_file);
    }

    // Verify file types have correct content
    let config_content = fs::read_to_string(&temp_path.join("agents/agents.yaml")).unwrap();
    ...
    #[cfg(unix)]
    {
        for script in ["setup.sh", "start-agents.sh", "attach-session.sh"] {
            let script_path = temp_path.join("agents/scripts").join(script);
...
/// Reusable helper function for walking directories
mod walkdir {
    use std::fs;
    use std::path::Path;
    ...
    impl WalkDir {
        pub fn new<P: AsRef<Path>>(path: P) -> walkdir::Result<Self> {
            Ok(Self {
                inner: walkdir::WalkDir::new(path)?,
            })
        }
    }
