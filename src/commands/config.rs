...
fn get_project_root_from_config(config_path: &Path) -> Option<PathBuf> {
    // The project root is the parent of the agents directory
    config_path.parent()?.parent().map(|p| p.to_path_buf())
}
...
impl SpriteConfig {
    ...
    /// Load configuration from the default path (searches up directory tree)
    pub fn load() -> Result<Self> {
        let config_path = find_project_config_path()
            .ok_or_else(|| SpriteError::config(
                "Could not find sprite configuration file. Run 'sprite init' in the project root first."
            ))?;
    ...
    /// Save configuration to the default path (saves to the project root)
    pub fn save(&self) -> Result<()> {
        let config_path = find_project_config_path().ok_or_else(|| {
            SpriteError::config("Could not find sprite project directory to save configuration.")
        })?;
    ...
    pub fn provision_worktrees(&self) -> Result<()> {
        ...
        for agent in &self.agents {
            ...
            // Remove existing worktree if directory doesn't exist but branch does
            if !worktree_path.exists() && git::branch_exists(&agent.branch)? {
                println!("    🔧 Removing stale worktree reference...");
                git::remove_worktree_reference(&worktree_path).with_context(|| {
                    format!("Failed to remove worktree reference for agent {}", agent.id)
                })?;
            }
    ...
    pub fn validate_workspaces(&self) -> Result<()> {
        ...
        // Get the project root to resolve relative workspace paths
        let config_path = match find_project_config_path() {
            Some(path) => path,
            None => {
                return Err(SpriteError::config(
                    "Could not find sprite configuration file to determine project root.",
                )
                .into());
            }
        };

        let project_root = match get_project_root_from_config(&config_path) {
            Some(root) => root,
            None => {
                return Err(SpriteError::config(
                    "Could not determine project root from configuration path.",
                )
                .into());
            }
        };
        ...
        for agent in &self.agents {
            ...
            // Convert to absolute path for comparison relative to project root
            let abs_worktree_path = if worktree_path.is_absolute() {
                ...
                project_root
                    ...
                    .canonicalize()
                    .unwrap_or_else(|_| {
                        // Fallback: use project root + relative path as-is
                        project_root.join(&worktree_path)
                    })
            };
    ...
    pub fn has_changed(&self) -> Result<bool> {
        let config_path = match find_project_config_path() {
            Some(path) => path,
            None => return Ok(false), // No config file, assume no changes
        };
    ...
    pub fn reload_if_changed(&mut self) -> Result<bool> {
        ...
        let config_path = match find_project_config_path() {
            Some(path) => path,
            None => return Ok(false), // No config file to reload
        };
    ...
    pub fn detect_changes(&self) -> Result<ConfigChanges> {
        let config_path = match find_project_config_path() {
            Some(path) => path,
            None => return Ok(ConfigChanges::new()), // No config file, no changes
        };
...
fn edit_config() -> Result<()> {
    let config_path = find_project_config_path().ok_or_else(|| {
        SpriteError::config(
            "Configuration file not found. Run 'sprite init' in the project root first.",
        )
    })?;
...
mod tests {
    ...
    fn test_provision_worktrees_validation() {
        ...
        match result {
            Ok(_) => {
                println!("Worktree provisioning succeeded");
            }
            Err(e) => {
                println!("Worktree provisioning failed as expected: {}", e);
            }
        }
    }
